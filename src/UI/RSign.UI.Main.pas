...
type
  ...
  private
    ...
    function MontarMensagemResumoFinal(const AResumo: TResumoFinalOperacao): string;
...
function TRSignMainForm.MontarMensagemResumoFinal(const AResumo: TResumoFinalOperacao): string;
begin
  Result := AResumo.ToMensagemUsuario;
end;
...
var
  LConfiguracao: TConfiguracaoAplicacao;
  LResumoFinalOperacao: TResumoFinalOperacao;
begin
  try
    LConfiguracao := MontarConfiguracaoDaTela;
    FOrchestratorClass.Execute(LConfiguracao);
    FConfiguracaoAtual := LConfiguracao;
    LResumoFinalOperacao := FOrchestratorClass.GetUltimoResumoOperacao;

    TDialogService.MessageDialog(
      MontarMensagemResumoFinal(LResumoFinalOperacao),
      TMsgDlgType.mtInformation,
      [TMsgDlgBtn.mbOK],
      TMsgDlgBtn.mbOK,
      0,
    ...
    on E: Exception do
    begin
      if Assigned(FOrchestratorClass) then
        LResumoFinalOperacao := FOrchestratorClass.GetUltimoResumoOperacao
      else
        LResumoFinalOperacao := TResumoFinalOperacao.Empty;

      TDialogService.MessageDialog(
        'Falha ao executar a operação: ' + E.Message + sLineBreak + sLineBreak +
        MontarMensagemResumoFinal(LResumoFinalOperacao),
        TMsgDlgType.mtError,
        [TMsgDlgBtn.mbOK],
        TMsgDlgBtn.mbOK,
